    ...
    public class ExpressionScanner
    {
        ...
        public ExpressionToken ExpectToken(ExpressionTokenType type)
        {
            ...
            throw new ExpressionException(
                message: $"Unable to parse language expression '{this.Expression}': expected token '{type}' and actual '{this.CurrentToken.Type}'.",
        ...
        public static IEnumerable<ExpressionToken> ParseExpression(string expression, TemplateErrorAdditionalInfo additionalInfo = null)
        {
            ...
            {
                throw new ExpressionException(
                    message: $"The language expression length limit exceeded. Limit: '{ExpressionConstants.ExpressionLimit}' and actual: '{expression.Length}'.",
            ...
            for (var currentPosition = 0; currentPosition < expression.Length; ++currentPosition)
            {
                ...
                switch (expression[currentPosition])
                {
                    ...
                    case '\'':
                        ...
                        {
                            throw new ExpressionException(
                                message: $"The language expression '{expression}' is not valid: the string constant at position '{startPosition}' is not terminated. String constants must be enclosed within a pair of single quotes.",
                                additionalInfo: additionalInfo);
                        }
                    ...
                    default:
                        if (expression[currentPosition] == '+' || expression[currentPosition] == '-' || char.IsDigit(expression[currentPosition]))
                        {
                            ...
                            {
                                throw new ExpressionException(
                                    message: $"The language expression '{expression}' is not valid: the string character '{expression[currentPosition]}' at position '{currentPosition}' is not expected.",
                                    additionalInfo: additionalInfo);
                            }
                            ...
                            if (tokenType == ExpressionTokenType.Integer)
                            {
                                ...
                                {
                                    throw new ExpressionException(
                                       message: $"The language expression '{expression}' is not valid: the value '{number}' at position '{startPosition}' cannot be converted to number.",
                                       additionalInfo: additionalInfo);
                                }
                            ...
                            else
                            {
                                ...
                                throw new ExpressionException(
                                    message: $"The language expression '{expression}' at position '{startPosition}' is not valid: float point numbers are not supported.",
                                    additionalInfo: additionalInfo);
                            }
                        }
